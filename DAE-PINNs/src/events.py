...
class ModelCheckPoint(Event):
    ...
    """
    def __init__(self, filepath, verbose=0, save_better_only=False, every=1, monitor="train loss", model_config=None):
    ...
    def on_epoch_completed(self):
        ...
        if self.save_better_only:
            ...
            if current < self.best:
                ...
                self.best = current
                self.model.save(self.filepath, verbose=0, model_config=self.model_config)
        else:
            self.model.save(self.filepath, verbose=self.verbose, model_config=self.model_config)
