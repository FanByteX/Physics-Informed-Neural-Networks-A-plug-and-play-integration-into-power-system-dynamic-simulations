...
class supervisor(object):
    ...
    def save(self, save_path, verbose=0, model_config=None):
        """
        save model to save_path
        Args:
            save_path: 保存路径
            verbose: 是否打印信息
            model_config: 模型配置字典（用于推理时重建网络结构）
        """
        if verbose > 0:
            print(
...
class State(object):
    ...
    def set_data_train(self, X, y, batch_size, shuffle=True):
        ...
        self.train_loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=shuffle, generator=generator)
