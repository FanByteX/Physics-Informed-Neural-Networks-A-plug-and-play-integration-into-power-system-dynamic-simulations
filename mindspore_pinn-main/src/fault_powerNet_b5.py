...
def main(args):
    print("starting...\n")
    print("=" * 80)
    print("🔧 线路故障仿真训练 - b=5 (阻抗增大)")
    print("=" * 80)
    print(f"故障类型: 电纳从 b=10 降至 b=5 (线路阻抗增大50%)")
    print(f"迁移学习: 从 b=10 最优模型继续训练")
    print("=" * 80)
    print()
    ...
    if use_cuda:
        ...
        # 从环境变量获取指定的 GPU 设备，默认使用 GPU 0
        cuda_visible_devices = os.environ.get('CUDA_VISIBLE_DEVICES', '0')
        gpu_id = int(cuda_visible_devices.split(',')[0]) if cuda_visible_devices else 0
        
        # 确保 GPU ID 在有效范围内
        if gpu_id >= num_gpus:
            gpu_id = 0
            print(f"Warning: Requested GPU {gpu_id} not available, using GPU 0")
    ...
    if args.use_input_layer:
        ...
        dyn_input_feature_layer = lambda x: x
    ...
    data = dae_data(X_train, X_test, args, device=str(device), func=power_net_dae)

    # start the supervisor
    super = supervisor(data, nn, device=str(device))
    
    # compile the supervisor
    optimizer = torch.optim.Adam(nn.parameters(), lr=args.lr)  # type: ignore
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="dae-pinns-fault-b5")

    # general
    parser.add_argument('--num-IRK-stages', type=int, default=100, help="number of RK stages")
    parser.add_argument('--log-dir', type=str, default="./logs/fault-b5/", help="log dir")
